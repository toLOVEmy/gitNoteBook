...
## 行为型模式
### 策略模式
- `成员`：抽象策略类（strategy），具体策略类（concrete strategy），环境类（context）
- `案例`：
对一个店铺管理人（环境类）来说，需要在不同节日选择不同的折扣方案（抽象策略类），如折扣A（具体策略类）...。
店铺管理人（环境类）聚合了折扣方案（抽象策略类），因此可以使用折扣方案，而折扣方案可以由折扣A，折扣B实现。
当折扣A实现时，店铺管理人使用折扣方案，效果就是折扣A。
当折扣B实现时，店铺管理人使用折扣方案，效果就是折扣B。
因此可以做到热插拔。
- `使用场景`：出现有多套方案时，仅方案内容不同。
